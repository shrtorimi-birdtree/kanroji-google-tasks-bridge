...
openapi/google-tasks-action-worker.yaml
...
## Cloudflare Worker 中継設定

Apps Script `ContentService` のHTTP 302リダイレクトでCustom GPT Actionsが停止する場合は、Cloudflare Workerを前段に置きます。GPT ActionsはWorkerの固定URLを呼び、WorkerがApps Script `/exec` へ中継します。

WorkerはGoogle Tasksの業務ロジックを持ちません。`health_check`、`list_today_tomorrow`、`audit_snapshot`、`create_task`、`update_due` をApps Scriptへ渡し、Apps Scriptから返ったJSONを `200 application/json` として返すだけです。

1. Cloudflare Workersで新しいWorkerを作成する
2. `cloudflare-worker/worker.js` の内容を貼り付ける
3. Apps Script Web App URLを変えた場合は、Workerの環境変数 `APPS_SCRIPT_EXEC_URL` に `/exec` で終わるURLを設定する
4. Workerをデプロイし、固定URLを取得する
5. Custom GPT Actionsに `openapi/google-tasks-action-worker.yaml` を貼り付ける
6. `servers.url` をWorker URLに差し替える
...
期待結果は `HTTP 200` と `content-type: application/json` です。本文で `ok: true` が返れば、WorkerがApps Scriptの302リダイレクトを吸収してJSONだけを返せています。
